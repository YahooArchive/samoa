...
public class ClusteringEvaluatorProcessor implements Processor {
	...
	public boolean process(ClusteringResultContentEvent result ) {		
                //evaluate
        ...
        public boolean process(ClusteringEvaluationContentEvent result ) {
            boolean ret = false;
	...
	private void concludeMeasurement(){
       ...
       private void printMeasures() {
		...
		for (int i = 0; i < measures.length; i++) {
			...
			sb.append("Mean ")
					.append(measures[i].getClass().getSimpleName() + ":"
							+ measures[i].getNumMeasures()).append("\n");
			for (int j = 0; j < measures[i].getNumMeasures(); j++) {
				sb.append(
						"[" + measures[i].getName(j) + "="
								+ measures[i].getLastValue(j)).append("] \n");
